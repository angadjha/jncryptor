...
import org.cryptonode.jncryptor.io.TrailerInputStream;

/**
 * Decrypts v2 data read as a stream.
 */
class AES256v2CipherInputStream extends InputStream {
   ...
   *          the password
   */
  AES256v2CipherInputStream(InputStream in, char[] password) {
   ...
   */
  AES256v2CipherInputStream(InputStream in, SecretKey decryptionKey,
      SecretKey hmacKey) {
  ...
  private void initializeStream() throws IOException {
    ...
    if (isPasswordEncrypted) {
      headerDataSize = AES256v2Ciphertext.HEADER_SIZE
          + AES256v2Ciphertext.ENCRYPTION_SALT_LENGTH
          + AES256v2Ciphertext.HMAC_SALT_LENGTH
          + AES256v2Ciphertext.AES_BLOCK_SIZE;
    } else {
      headerDataSize = AES256v2Ciphertext.HEADER_SIZE
          + AES256v2Ciphertext.AES_BLOCK_SIZE;
    }
    ...
    if (version != AES256v2Ciphertext.EXPECTED_VERSION) {
      throw new IOException(String.format("Expected version %d but found %d.",
          AES256v2Ciphertext.EXPECTED_VERSION, version));
    }

    byte options = headerData[offset++];

    if (isPasswordEncrypted) {
      if (options != AES256v2Ciphertext.FLAG_PASSWORD) {
        throw new IOException("Expected password flag missing.");
      }

      byte[] decryptionSalt = new byte[AES256v2Ciphertext.ENCRYPTION_SALT_LENGTH];
      System.arraycopy(headerData, offset, decryptionSalt, 0,
          decryptionSalt.length);
      offset += decryptionSalt.length;

      byte[] hmacSalt = new byte[AES256v2Ciphertext.HMAC_SALT_LENGTH];
    ...
    byte[] iv = new byte[AES256v2Ciphertext.AES_BLOCK_SIZE];
    System.arraycopy(headerData, offset, iv, 0, iv.length);

    trailerIn = new TrailerInputStream(in, AES256v2Ciphertext.HMAC_SIZE);

    try {
      Cipher decryptCipher = Cipher
          .getInstance(AES256v2Cryptor.AES_CIPHER_ALGORITHM);
      ...
      mac = Mac.getInstance(AES256v2Cryptor.HMAC_ALGORITHM);
      mac.init(hmacKey);
