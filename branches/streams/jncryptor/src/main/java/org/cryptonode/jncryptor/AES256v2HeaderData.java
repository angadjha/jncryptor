...
class AES256v2HeaderData {
  ...
  private static final int SIZE_WITH_PASSWORD = AES256v2Ciphertext.HEADER_SIZE
      + AES256v2Ciphertext.ENCRYPTION_SALT_LENGTH
      + AES256v2Ciphertext.HMAC_SALT_LENGTH + AES256v2Ciphertext.AES_BLOCK_SIZE;

  private static final int SIZE_WITHOUT_PASSWORD = AES256v2Ciphertext.HEADER_SIZE
  ...
  public AES256v2HeaderData(byte[] data) throws InvalidDataException {
    ...
    if (data.length < AES256v2Ciphertext.HEADER_SIZE) {
    ...
    if (options != 0x00 && options != AES256v2Ciphertext.FLAG_PASSWORD) {
      throw new InvalidDataException("Unrecognised bit in the options byte.");
    }

    // If the password bit is set, we can expect salt values
    isPasswordBased = ((options & AES256v2Ciphertext.FLAG_PASSWORD) == AES256v2Ciphertext.FLAG_PASSWORD);
    ...
    if (isPasswordBased) {
      encryptionSalt = new byte[AES256v2Ciphertext.ENCRYPTION_SALT_LENGTH];
      System.arraycopy(data, index, encryptionSalt, 0, encryptionSalt.length);
      index += encryptionSalt.length;

      hmacSalt = new byte[AES256v2Ciphertext.HMAC_SALT_LENGTH];
    ...
    iv = new byte[AES256v2Ciphertext.AES_BLOCK_SIZE];
